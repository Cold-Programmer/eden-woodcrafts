  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        <Link href="/staff/products/new" className="rounded-full bg-forest px-4 py-2 text-sm font-medium text-warmwhite">
          + New Product
        </Link>
      </div>
      <p className="mt-1 text-sm text-charcoal/50">
        You can add and edit products. Removing a product from the storefront is an admin-only action.
      </p>
      ...
      <div className="mt-6 overflow-x-auto rounded-xl2 border border-wood/10 bg-white">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {products.map((p) => (
              <tr key={p.id} className="border-b border-wood/5">
                ...
                <td className="px-4 py-3">
                  <Link href={`/staff/products/${p.id}/edit`} className="text-forest hover:underline">Edit</Link>
                </td>
              </tr>
            ))}
          </tbody>
